...
b. $f(x) = \frac{1}{\sqrt{2\pi}}\exp\left(-x^2/2\right)$
c. $f(x; \nu_1, \nu_2) = \frac{\Gamma\left(\frac{\nu_1+\nu_2}{2}\right)\left(\frac{\nu_1}{\nu_2}\right)^{\nu_1/2}}{\Gamma(\nu_1/2)\Gamma(\nu_2/2)}\frac{x^{\nu_1/2-1}}{(1 + x\nu_1/\nu_2)^{(\nu_1+\nu_2)/2}}$
d. $f(x; k) = \frac{1}{2^{k/2}\Gamma(k/2)}x^{k/2-1}e^{-x/2}$
...
a. A value calculated from data.
...
::: {.content-visible when-profile="book"}
In this section, we'll just lay out some basic facts about some distributions. I try to also justify why these facts are important for this course! Otherwise, the following examples should act as a reference.
:::
...
1. The code below demonstrates one of the topics covered in this lecture. Which is it?
...
2. True or False: As $n$ approaches infinity, the sampling distribution of the **variance** approaches a normal distribution?

<details>
<summary>**Solution**</summary>

If $n = 3$, then the variance clearly does not follow a normal distribution. Instead, it is a transformation of a Chi-Square distribution. What happens as n gets larger? What if the true sd gets *smaller*? What if we use a uniform distribution, or something else?
