...
For plots on the right, Red line is the mean of `body_mass_g` when speciesis 4, 6, and 8, respectively.
...
If we have `cyl` and `flipper_length_mm` in the model, we get the following:
...
$$
y = \beta_0 + \beta_1I(Gentoo) + \beta_2I(4) + \beta_3flipper_length_mm + \beta_4flipper_length_mmI(Gentoo) + \beta_5flipper_length_mmI(4)
$$
