...
- $R^2$, or adjusted $R^2$
    - Higher is better, but $R^2$ increases as we add predictors\lspace
...
```{webr-r}
last_digit_of_student_number <- 0
Hitters <- read.csv(paste0("https://raw.githubusercontent.com/DB7-CourseNotes/ST362-STAR/main/data/Hitters_", last_digit_of_student_number, ".csv"))

mymodel <- lm(Salary ~ ., data = Hitters)
par(mfrow = c(2, 2))
plot(mymodel)

# Find the best model you can come up with!
# Try ?step to look at automated subset selection!
# You might also want to try Salary ~ .^2, which includes all interaction
# terms, but you should probably use forward selection on that (explain why!)
```
:::
...
n <- 100
p <- 30
# Generate 30 uncorrelated predictors
X <- matrix(ncol = p, nrow = n, data = runif(n * p, 0, 10))
# y is uncorrelated with the predictors
y <- matrix(ncol = 1, nrow = n, data = rnorm(n, 0, 3))
mydf <- cbind(y = y, X) |> as.data.frame()
names(mydf)[1] <- "y"
step(lm(y ~ ., data = mydf), direction = "backward", trace = 0) |>
    coef()
```
:::
...
a. They do choose the same model.
b. Because the order that the predictors enter the model matters.
c. Because the best model is different depending on which approach you use.
...
c. A few of them.
d. What? What kind of question is that? How could I possibly know without seeing the context of the problem?!?
