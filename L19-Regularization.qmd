...
- 15 people still haven't joined groups for the Final Project!\lspace
- Final Lab Wednesday
    - Class in lab again?\lspace
...
- Minimize the loss function with respect to the parameters of interest.\lspace
- For the same parameters, there can be many loss functions.\lspace
- Other names:
...
Regularization is a method for *constraining* the parameters to prevent overfitting.
...
$$
||\underline\beta||_p = \left(\sum_{j=1}^q|\beta_q|^p\right)^{1/p}
$$

- When $p = 2$, this is the Euclidean distance.
    - Pythagoras strikes again!\lspace
- When $p = 1$, this is the sum of absolute values.\lspace
...
\pspace _Predictors **must** be standardized for regularization to be valid!_
...
- LASSO will set coefficients to 0.
    - "Least Absolute Shrinkage and Selection Operator"\lspace
...
::: {.content-visible when-profile="book"}
For the curious, [converting between $s$ and $\lambda$ is not straightforward](https://stats.stackexchange.com/questions/320028/lasso-relationship-between-lambda-and-t).
:::
...
\vspace{2cm}
- As $\lambda$ increases, more coefficients are allowed to be non-zero.\lspace
...
```{webr-r}
#| eval: true
# Run this code once and it will install packages,
# run again and the package installation messages will disappear.
library(glmnet)
library(palmerpenguins)

# glmnet requires separate matrix for X and y,
# NAs in X might get removed, making X and y have
# incompatible dimensions, therefore we must remove
# NAs ourselves.
peng <- na.omit(penguins)

# glmnet requires a matrix - it will not create dummy 
# variables for you. The easiest way to do this is to
# get lm() to do it for us, then extract the results.
mylm <- lm(
  formula = body_mass_g ~ 
    (flipper_length_mm + bill_depth_mm + bill_length_mm) * species + 
    flipper_length_mm * island,
  data = peng)
print("Coefficients of the linear model:")
summary(mylm)$coef # Lots of predictors are not significant.

# The [, -1] removes the first column, which is just 1s
x <- model.matrix(mylm)[, -1]
y <- peng$body_mass_g

# Estimate lambda via cross-validation
lambda_estimates <- cv.glmnet(x = x, y = y)
plot(lambda_estimates)

# Fit the model with the lambda that minimizes CV error
# (Also try lasso_estimates$lambda.1se; what do you notice?)
mylasso <- glmnet(x = x, y = y,
  lambda = lambda_estimates$lambda.min)
cat("\n\n\n") # Empty lines to separate R output
...
a. Depending on the context of the problem, we might have a specific maximum value for the sum of the coefficients.
b. We want to avoid overfitting the data, and restricting the parameter stops us from modelling irrelevant patterns.
c. Because in any given regression problem there are always predictors that should have a slope of 0.
d. Because some parameter estimates are impossible.
...
a. LASSO has a tighter restriction on the parameters than Ridge.
b. LASSO will set parameters to 0, whereas Ridge will just shrink them towards 0 without making them exactly 0.
c. Because coefficients are set to 0, LASSO has higher variance in its estimates. 
d. The only difference is the norm that they use, which has no meaningful impact on the results.
...
a. The out-of-sample prediction error. 
b. The bias in the estimates.
c. The loss function (MSE).
...
a. When you want un unbiased estimate of the population parameters.
b. When you want to avoid overfitting.
d. When you want to be regular/normal/usual/typical.
...
a. When you want to do subset selection in a way that minimizes out-of-sample prediction error.
b. When you want most of the coefficients to be 0.
c. When you want to minimize out-of-sample prediction error at all costs.
d. When you only want coefficients with significant p-values left over in your model.
...
- The `glm` in `glmnet` is because it fits all GLMs.
    ...
    - The `family = binomial` argument works as in `glm()`
        - However, `family = "binomial"` is an optimized version.\lspace
- The `net` in `glmnet` refers to elasticnet.
    - Next slide or two.

### Elastic Net: Like a Lasso/Ridge, but more "flexible" {.t}
...
res <- cbind(
        coef_mat, 
        apply(coef_mat, 1, function(x) abs(x[2] - x[1]))
    ) |> 
    round(3)
