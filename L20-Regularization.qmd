...
# NAs in X might get removed, making X and y have incompatible dimensions.
# Therefore we must remove NAs ourselves.
peng <- na.omit(penguins)

# glmnet requires a matrix - it will not create dummy variables for you.
# The easiest way to do this is to get lm() to do it for us,
# then extract the results.
mylm <- lm(body_mass_g ~ (flipper_length_mm + bill_depth_mm + bill_length_mm) * species + flipper_length_mm * island, data = peng)
summary(mylm) # Lots of predictors are not significant.

# The [, -1] removes the first column, which is just the 1s column.
