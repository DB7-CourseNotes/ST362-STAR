...
The x axis is the potential values of $\beta_1$ and the y is $\beta_2$. The colours represent the value of the SSE, $\sum_{i=1}^n(y_i - \beta_1x_{i1} - \beta_2x_{i2})^2$
...
server <- function(input, output) {
    ...
    make_data <- reactive({
        ...
        X <- mvrnorm(100, mu = c(0, 0), 
            Sigma = matrix(c(2, input$corr, input$corr, 2), ncol = 2))
        x1 <- X[, 1]
        x2 <- X[, 2]
